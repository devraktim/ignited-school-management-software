<?php 
    ...
    class ExamAttendenceController extends CI_Controller {
        ...
        public function index() {
            ...
            if($class_id && $section_id) {
                ...
                $this->load->view("academics/exam_attendence", array("classes" => $classes, "sections" => $sections, "students" => $students));
            }
            else if($class_id) {
                $sections = $this->ClassSection->get_sections($academy_session_id, $class_id);
                ...
                $this->load->view("academics/exam_attendence", ["classes" => $classes]);
            }   
        }
        ...
        public function get_students() {
            ...
            $class_teacher_can_enter_attendance = $settings['class_teacher_can_enter_attendance'];
            ...
            foreach($subjects as $subject) {
                ...
                foreach($rows as $row) {
                    if(!in_array($row["student_id"], $student_list))  {
                        ...
                        $d = $this->ExamAttendence->get_attendence(["class_id" => $_POST["class_id"], "section_id" => $_POST["section_id"], "exam_id" => $_POST["exam_id"], "student_id" => $row["student_id"]]);
            ...
            $this->load->view("academics/exam_attendence", [
                "students"          => $students,
                "total_days"        => $students[0]["total_days"],
                "classes"           => $classes,
                "sections"          => $sections,
                "exams"             => $exams,
                "class_id"          => $_POST["class_id"],
                ...
                "class_teacher_can_enter_attendance" => $class_teacher_can_enter_attendance
            ]);
        }

        public function exam_attendence_store() {
            $ids = $this->input->post("ids");
            $attendences = $this->input->post("present");
            ...
            for($i = 0 ; $i < count($ids) ; $i++) {
                $data[] = [
                    ...
                    "exam_id" => $exam_id,
                    "student_id" => $ids[$i],
                    "present" => $attendences[$i],
                    "total_days" => $total_days,
                    "session_id" => $academy_session_id
                ];
            }

            $this->ExamAttendence->attendence_store($data);

            return redirect(base_url() . "academics/exam-attendence-entry");
        }
    }