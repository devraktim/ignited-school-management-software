            ...
            <table class="table table-bordered">
                <thead>
                    <tr>
                        ...
                        <th class="text-center">UT2 <br> (20)</th>
                        <th class="text-center">FT <br> (80)</th>
                        <th class="text-center">FINAL <br> (100)</th>
                        <th class="text-center">HY <br> (100)</th>
                        <th class="text-center">ANNUAL <br> (100)</th>
                ...
                <tbody>
                    ...
                    $english_avg = ceil(
                        (
                            ceil(($student['final_totals']['s1'] + $student['final_totals']['s2']) / 2) +
                            ceil(($student['mid_totals']['s1'] + $student['mid_totals']['s2']) / 2)
                        ) / 2
                    );
                    ...
                    <?php foreach (array_filter($student['subject_type_ids'], function($id) { return $id != 1 && $id != 2; }) as $id): ?>
                        <?php
                            ...
                            $ut2 = $student['ut2_marks']["s$id"] ?? 0;
                            $finalt = $student['final_marks']["s$id"] ?? 0;
                            $final_total = $student['final_totals']["s$id"] ?? 0;
                            $mid_total = $student['mid_totals']["s$id"] ?? 0;
                            $annual_marks = $student['annual_marks']["s$id"] ?? 0;
                
                            // Grade logic
                            if ($annual_marks >= 91)       $grade = '1';
                            elseif ($annual_marks >= 81)   $grade = '2';
                            elseif ($annual_marks >= 71)   $grade = '3';
                            elseif ($annual_marks >= 61)   $grade = '4';
                            elseif ($annual_marks >= 51)   $grade = '5';
                            elseif ($annual_marks >= 41)   $grade = '6';
                            elseif ($annual_marks >= 35)   $grade = '7';
                            elseif ($annual_marks >= 25)   $grade = '8';
                            elseif ($annual_marks >= 1)    $grade = '9';
                            else                    $grade = '-';
                        ?>
                        <tr>
                            <td><?= $subject_name ?></td>
                            <td class="text-center"><?= $ut2 ?></td>
                            <td class="text-center"><?= $finalt ?></td>
                            <td class="text-center"><?= $final_total ?></td>
                            <td class="text-center"><?= $mid_total ?></td>
                            <td class="text-center"><?= $annual_marks ?></td>
                    ...
                    <?php if (!empty($student['special_subject_type_ids'])): ?>
                        ...
                        <?php foreach ($student['special_subject_type_ids'] as $index => $id): ?>
                            <?php
                                $key = "sps" . ($index + 1);
                                $special_subject_name = $student['special_subjects'][$key] ?? 'Subject';
                                $special_ut2 = $student['special_ut2_marks'][$key] ?? 0;
                                $special_finalt = $student['special_final_marks'][$key] ?? 0;
                                $special_final_total = $student['special_final_totals'][$key] ?? 0;
                                $special_mid_total = $student['special_mid_totals'][$key] ?? 0;
                                $special_annual_marks = $student['special_annual_marks'][$key] ?? 0;
                
                                // Grade logic
                                if ($special_annual_marks >= 91)       $grade = '1';
                                elseif ($special_annual_marks >= 81)   $grade = '2';
                                elseif ($special_annual_marks >= 71)   $grade = '3';
                                elseif ($special_annual_marks >= 61)   $grade = '4';
                                elseif ($special_annual_marks >= 51)   $grade = '5';
                                elseif ($special_annual_marks >= 41)   $grade = '6';
                                elseif ($special_annual_marks >= 35)   $grade = '7';
                                elseif ($special_annual_marks >= 25)   $grade = '8';
                                elseif ($special_annual_marks >= 1)    $grade = '9';
                                else                    $grade = '-';
                            ?>
                            <tr>
                                <td><?= $special_subject_name ?></td>
                                <td class="text-center"><?= $special_ut2 ?></td>
                                <td class="text-center"><?= $special_finalt ?></td>
                                <td class="text-center"><?= $special_final_total ?></td>
                                <td class="text-center"><?= $special_mid_total ?></td>
                                <td class="text-center"><?= $special_annual_marks ?></td>
                                <td class="text-center"><?= $grade ?></td>
                            </tr>
                        <?php endforeach; ?>
                    <?php endif; ?>
