<?php $this->load->view("inc/app_header.php"); ?>
    ...
    <div class="card card-flush h-xl-100 mb-5">
        <div class="card-body py-9">
            <form action="<?php echo base_url() ?>academics/evolution-entry" method="POST">
                <div class="row">
    ...
    <?php if(isset($students)) { ?>
        <?php if(count($students) > 0) { ?>
            <div class="row">
                <form action="<?php echo base_url()?>academics/evolution-store" method="POST">
                    <div class="col-md-12">
                                    ...
                                    <table class="table table-bordered">
                                        <thead>
                                            <tr class="table-dark text-light">
                                                ...
                                                <th class="text-nowrap sticky-header sticky-column-2" style="z-index: 5 !important;">Student No</th>
                                                <th class="text-nowrap sticky-header sticky-column-3" style="z-index: 5 !important;">Roll No</th>
                                                <th class="text-nowrap sticky-header sticky-column-4" style="z-index: 5 !important;">Name</th>
                                                <?php foreach($papers as $paper) { ?>
                                                    <th class="text-nowrap sticky-header"><?php echo $paper['name'] ?></th>
                                                <?php } ?>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            <?php $i = 0; foreach($students as $student) { $i++; ?>
                                                <tr>
                                                    <td class="table-warning text-dark p-2 sticky-column-1"><?php echo $i; ?></td>
                                                    <td class="text-nowrap sticky-column-2" style="background-color: #fbffd1 !important;"><?php echo $student["student_no"]?> </td>
                                                    <td class="text-nowrap sticky-column-3" style="background-color: #fbffd1 !important;"><?php echo $student["roll_no"]?> </td>
                                                    <td class="text-nowrap sticky-column-4" style="background-color: #fbffd1 !important;"><?php echo $student["f_name"] . " " . $student["m_name"] . " " . $student["l_name"] ?></td>
                                                    <?php $j = -1; foreach($papers as $paper) { $j++; ?>
                                                        <td class="text-nowrap">
                                                            <select class="form-select" name="grades_<?php echo $student['id']?>[]">
                                                                <option value="">Please Select</option>
                                                                <?php foreach($grades as $grade) { ?>
                                                                    <option value="<?php echo $grade['name'] ?>" <?php if($grade['name'] == $student['grade'][$j]) {echo "selected";}?>><?php echo $grade['name'] ?></option>
                                                                <?php } ?>
                                                            </select>
                                                        </td>
                                                    <?php } ?>
                    ...
                    <input type="text" class="form-control d-none" name="exam_id"      value="<?php echo $exam_id ?>">
                     ...
                     <?php if($this->session->user['permissions'][0]['academics_module'] != "VIEWER") { ?>
                        <?php if($class_teacher_can_enter_personal_evaluation == '1') { ?>
    ...
    <script>
        $("#class_id").change(function(event) {
            ...
            fetch("<?php echo base_url('academics/evolution-entry?class_id=') ?>" + $("#class_id").val())
