...
<html lang="en">
    ...
    <?php
        // Assuming $records is your array of data (already fetched)
        $class_wise_data = [];
        foreach ($records as $record) {
            $class_wise_data[$record['class_name']] = intval($record['total_students']);
        }
        // Convert data to JSON for use in JavaScript
        $class_wise_data = json_encode($class_wise_data);
    ?>
        ...
        ?>
            ...
            <div class="BigHeader" style="width:90%; margin: 0 auto; margin-top:20px">Nationality Wise Breakup <?php echo date('Y', strtotime($this->session->academy_session['current_session']['start'])) ?></div>
